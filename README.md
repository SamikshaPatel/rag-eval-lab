...
│   ├── rag_grounding_v1.txt     # RAG grounding prompt — Zephyr corpus
│   ├── rag_grounding_v2.txt     # RAG grounding prompt — Northstar corpus (exception + inference-aware)
...
│   ├── mini_eval_northstar.py   # Dev utility: run 4 targeted questions without LangSmith
│   └── score_single_question.py # Dev utility: run all 10 DeepEval metrics on one question
├── reports/
│   ├── Zephyr_reports/          # Zephyr corpus eval reports
...
│   │   └── zephyr_framework_comparison_ragas_vs_deepeval.html
│   └── Northstar_reports/       # Northstar corpus eval reports
│       ├── northstar_eval_k3_chunk400o80_llama31_qwen25.html
│       ├── northstar_eval_comparison_chunk400_vs_chunk200.html
│       ├── northstar_eval_comparison_chunk400_chunk200_rerank.html
│       └── northstar_eval_comparison_run2_vs_run4.html
├── docs/screenshots/            # LangSmith UI screenshots referenced in learning path
├── chroma_db_zephyr/            # Persisted vector store (generated by ingest_zephyr.py)
├── chroma_db_northstar/         # Persisted vector store (generated by ingest_northstar.py)
...
| `judge_correctness_v1.txt` | `PROMPT_JUDGE_CORRECTNESS` | `src/eval_langsmith.py`, `src/eval_langsmith_northstar.py` | `_CORRECTNESS_PROMPT` | `make_correctness_evaluator()` — scores factual accuracy vs reference |
| `judge_completeness_v1.txt` | `PROMPT_JUDGE_COMPLETENESS` | `src/eval_langsmith.py`, `src/eval_langsmith_northstar.py` | `_COMPLETENESS_PROMPT` | `make_completeness_evaluator()` — scores coverage of reference facts |
...
| **Chunking** | `src/ingest_zephyr.py` + `src/config.py` | Chunk size is a tuning knob *and* a common bug source — change `CHUNK_SIZE` in `config.py`, then re-run `ingest_zephyr.py` |
...
→ Screenshot: [`docs/screenshots/1_langsmith_northstar_experiment_comparison.png`](docs/screenshots/1_langsmith_northstar_experiment_comparison.png)
...
| **Run 4** — chunk200+fixes | 200/40 | None | v2 | Multi-query (roles) | 0.89 | 0.92 | 0.45 | 0.71 | 0.84 | 0.44 | Prompt v2 (exception + universal-rule awareness) fixed NSB-023/024; multi-query retrieval per role fixed NSB-033. Full comparison → [`northstar_eval_comparison_run2_vs_run4.html`](reports/Northstar_reports/northstar_eval_comparison_run2_vs_run4.html) |
